...
$tfrom  = $_GET['tfrom']  ?? date('Y-m-01');
$tto    = $_GET['tto']    ?? date('Y-m-d');
...
switch ($period) {
    ...
    case 'custom':
        $dateFrom = $tfrom;
        $dateTo   = $tto;
        $periodLabel = 'Custom Range: ' . date('d M Y', strtotime($tfrom)) . ' – ' . date('d M Y', strtotime($tto));
        break;
    case 'all':
        $dateFrom = $dateTo = null;
...
if (($_GET['export'] ?? '') === 'excel') {
    ...
    fwrite($out, "\xEF\xBB\xBF"); // UTF-8 BOM so Excel renders the currency symbol/special characters correctly
    fputcsv($out, ['Transactions Report']);
    fputcsv($out, [$periodLabel]);
    fputcsv($out, ['Generated: ' . date('d M Y H:i')]);
    fputcsv($out, []);
    fputcsv($out, ['Date', 'Type', 'Description / Details', 'Amount', 'Bank Reference']);
    ...
    fputcsv($out, []);
    fputcsv($out, ['', '', 'TOTAL (' . count($transactions) . ' transaction' . (count($transactions) === 1 ? '' : 's') . ')', number_format($totalAmount, 2), '']);
    fclose($out);
    exit;
}
...
<div class="card" style="margin-bottom:20px">
  ...
  <form method="GET" id="txFilterForm" style="display:flex;gap:12px;align-items:flex-end;flex-wrap:wrap">
    ...
    <div class="form-group" id="tfromGroup" style="margin:0;display:<?= $period==='custom'?'':'none' ?>">
      <label>From</label>
      <input type="date" name="tfrom" value="<?= h($tfrom) ?>">
    </div>
    <div class="form-group" id="ttoGroup" style="margin:0;display:<?= $period==='custom'?'':'none' ?>">
      <label>To</label>
      <input type="date" name="tto" value="<?= h($tto) ?>">
    </div>
    <button type="submit" class="btn btn-primary">Apply</button>
    <a href="?<?= http_build_query(array_merge($_GET, ['export'=>'excel'])) ?>" class="btn btn-ghost">⬇️ Download Excel</a>
  </form>
</div>
...
function toggleTxPeriod() {
    ...
    document.getElementById('tfromGroup').style.display  = period === 'custom'  ? '' : 'none';
    document.getElementById('ttoGroup').style.display    = period === 'custom'  ? '' : 'none';
}
</script>
