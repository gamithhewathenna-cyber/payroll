...
function handleReadOnlyAction($db, $type, $d, $sym) {
    ...
    if ($type === 'get_invoice_details') {
        $q     = trim($d['query'] ?? '');
        $month = trim($d['month'] ?? '');
        if (!$q) return ['message' => "Please tell me the invoice number or client name to look up.", 'link' => null];

        $sql = "SELECT i.*, c.company_name, c.contact_name FROM invoices i JOIN clients c ON c.id=i.client_id
                WHERE (i.invoice_number LIKE ? OR c.company_name LIKE ?)";
        $params = ["%{$q}%", "%{$q}%"];
        if ($month) { $sql .= " AND DATE_FORMAT(i.issue_date,'%Y-%m')=?"; $params[] = $month; }
        $sql .= " ORDER BY i.issue_date DESC LIMIT 10";
        $stmt = $db->prepare($sql);
        $stmt->execute($params);
        $rows = $stmt->fetchAll();

        if (!$rows) return ['message' => "No invoice found matching \"{$q}\".", 'link' => SITE_URL.'/invoices.php'];

        if (count($rows) > 1) {
            $list = implode("\n", array_map(fn($r) => "- **{$r['invoice_number']}** — {$r['company_name']} — ".date('d M Y', strtotime($r['issue_date'])), $rows));
            return ['message' => "I found multiple invoices matching \"{$q}\":\n\n{$list}\n\nCan you give me the exact invoice number?", 'link' => SITE_URL.'/invoices.php'];
        }
        ...
        $tSym      = $isForeign ? ($curSymbols[$invCur] ?? $invCur) : $sym;
        $conv      = fn($amtLKR) => $isForeign ? $amtLKR / $invRate : $amtLKR;

        $itemsStmt = $db->prepare("SELECT description, quantity, unit_price, amount, item_type FROM invoice_items WHERE invoice_id=? ORDER BY sort_order");
        $itemsStmt->execute([$inv['id']]);
        $items = $itemsStmt->fetchAll();

        $isQuote  = $inv['invoice_type'] === 'quotation';
        $docLabel = $isQuote ? 'Quotation' : 'Invoice';
        $badge    = ['draft'=>'📝 Draft','sent'=>'📤 Sent','paid'=>'✅ Paid','overdue'=>'⚠️ Overdue','cancelled'=>'🚫 Cancelled'][$inv['status']] ?? ucfirst($inv['status']);

        $msg = "📄 **{$docLabel} {$inv['invoice_number']}** — {$inv['company_name']}\nStatus: {$badge}\nIssued: " . date('d M Y', strtotime($inv['issue_date']));
        if ($inv['due_date']) $msg .= " · " . ($isQuote ? 'Valid Until' : 'Due') . ": " . date('d M Y', strtotime($inv['due_date']));
        $msg .= "\n\n";

        if ($items) {
            $msg .= "**Items:**\n";
            foreach ($items as $it) {
                $desc    = trim(explode('|||', $it['description'], 2)[0]);
                $amtDisp = ($isForeign && $it['item_type'] !== 'expense') ? $it['quantity'] * $it['unit_price'] : $it['amount'];
                $msg .= "- {$desc} — {$tSym} " . number_format($amtDisp, 2) . "\n";
            }
            $msg .= "\n";
        }

        $msg .= "Subtotal: {$tSym} " . number_format($conv($inv['subtotal']), 2) . "\n";
        if ($inv['discount_pct'] > 0) $msg .= "Discount ({$inv['discount_pct']}%): -{$tSym} " . number_format($conv($inv['discount_amt']), 2) . "\n";
        if ($inv['tax_pct'] > 0) $msg .= "Tax ({$inv['tax_pct']}%): {$tSym} " . number_format($conv($inv['tax_amt']), 2) . "\n";
        $msg .= "**Total: {$tSym} " . number_format($conv($inv['total']), 2) . "**\n";
        ...
        if ($inv['paid_date']) $msg .= "Paid On: " . date('d M Y', strtotime($inv['paid_date'])) . "\n";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'execute') {
    ...
    try {
        if ($type === 'create_invoice') {
            $clientName = trim($d['client_name'] ?? '');
            if (!$clientName) throw new Exception('A client name is required to create an invoice.');
            if (empty($d['items'])) throw new Exception('An invoice needs at least one line item — please describe what this invoice is for.');
            ...
            $client = $cStmt->fetch();
            if (!$client) throw new Exception("Client '{$clientName}' not found. Please add them to the Clients page first.");
            ...
            foreach (($d['items'] ?? []) as $item) {
                $desc = trim($item['desc'] ?? '');
                if ($desc === '') continue; // skip malformed/empty line items rather than inserting a blank row
                $qty    = (float)($item['qty'] ?? 1);
                $price  = (float)($item['price'] ?? 0);
                $amtLKR = round($qty * $price * $rate, 2);
                $subtotal += $amtLKR;
                $items[] = [$desc, $item['subdesc'] ?? '', $qty, $price, $amtLKR];
            }
            if (empty($items)) throw new Exception('An invoice needs at least one line item with a description — please describe what this invoice is for.');
