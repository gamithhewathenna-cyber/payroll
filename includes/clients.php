...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add') {
        try {
            $db->prepare("INSERT INTO clients (company_name,contact_name,email,phone,address,industry,status,notes) VALUES (?,?,?,?,?,?,?,?)")
               ->execute([trim($d['company_name']),trim($d['contact_name']??''),trim($d['email']??''),trim($d['phone']??''),trim($d['address']??''),trim($d['industry']??''),$d['status']??'active',trim($d['notes']??'')]);
            setFlash('success', 'Client added.');
        } catch (Exception $e) { setFlash('error', 'Error: ' . $e->getMessage()); }
    } elseif ($action === 'edit') {
        $db->prepare("UPDATE clients SET company_name=?,contact_name=?,email=?,phone=?,address=?,industry=?,status=?,notes=? WHERE id=?")
           ->execute([trim($d['company_name']),trim($d['contact_name']??''),trim($d['email']??''),trim($d['phone']??''),trim($d['address']??''),trim($d['industry']??''),$d['status']??'active',trim($d['notes']??''),$id]);
...
<div class="card" style="padding:0;overflow:hidden">
  <div class="table-wrap mob-card-table">
    <table>
      <thead><tr><th>Company</th><th>Contact</th><th>Email</th><th>Phone</th><th>Industry</th><th>Expenses</th><th>Status</th><th>Actions</th></tr></thead>
      <tbody>
    ...
    <div class="modal-body">
      <form method="POST" action="?action=add">
        <div class="form-grid">
          ...
          <div class="form-group full"><label>Address</label><textarea name="address" rows="2" placeholder="Client address..."></textarea></div>
    ...
    <div class="modal-body">
      <form method="POST" action="?action=edit&id=<?= $editRow['id'] ?>">
        <div class="form-grid">
          ...
          <div class="form-group full"><label>Address</label><textarea name="address" rows="2"><?= h($editRow['address']) ?></textarea></div>
